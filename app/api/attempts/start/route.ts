...
import { DEMO_ATTEMPT_COOKIE_NAME } from "@/lib/config";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.demo) {
      ...
      const response = NextResponse.json({ ok: true, attemptId, demo: true });
      const expiresAt = new Date(Math.min(Date.parse(session.sessionExpiresAt), Date.parse(session.codeExpiresAt))).toISOString();
      setPrivateCookie(response, DEMO_ATTEMPT_COOKIE_NAME, attemptId, expiresAt);
      return response;
    }

    const supabase = createServiceClient();
    const { data, error } = await supabase.rpc("start_test_attempt", { p_code_id: session.redeemCodeId });
    if (error) {
